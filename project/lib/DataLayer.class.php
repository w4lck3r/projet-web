...
class DataLayer {
	...
	function favorie($insee,$login){
		$sql = "select * from favories where login = :login and insee = :insee";
		...
		$res= $stmt->fetch();
		if ($res) {
		    produceError("Deja dans la liste");
		}else{
		    $sql = "insert into favories (login, insee) values (:login, :insee)";
		    $stmt = $this->connexion->prepare($sql);
		    $stmt->bindValue('login', $login);
    	    $stmt->bindValue('insee', $insee);
		    $stmt->execute();
		    $res= $stmt->fetch();
		    produceResult("C'est fait");
		}
	}
	
	function Afficherfavorie($login){
		$sql = "select * from favories where login = :login";
		...
		if ($res) {
		    foreach ($res as $single){
		        $sql2 = "select nom from communes where insee = :insee";
		        $stmt2 = $this->connexion->prepare($sql2);
		        $stmt2->bindValue('insee', $single['insee']);
		        $stmt2->execute();
		        $res2= $stmt2->fetch();
		        if ($res2){
		            echo "<li>".$res2."</li>";
		        }else{
		            produceError("Commune n'existe plus");
		        }
		    }
		}else{
		    produceError("Vide");
...
EOD;
    ...
    function authentification(string $login, string $password) : ?Identite{ // version password hash
			...
			if ($res) {
			    $_SESSION['ident'] = new Identite($res['login'], $res['nom'], $res['prenom']);
    ...
    */
    function createUser(string $login, string $password, string $nom, string $prenom) : bool	 {
		...
		try{
			$stmt->execute();produceResult(utf8ize($communes));
